...
/**
 * @file /irobot_keyop/src/keyop_core.cpp
 *
...
bool KeyOpCore::init()
{
  ...
  while (!connected)
  {
    ...
    {
      ROS_WARN_STREAM("KeyOp: could not connect to irobot_create driver, trying again after 500ms...");
      try
      {
        millisleep(500);
      }
  ...
  if (!connected)
  {
    ...
    ROS_ERROR("KeyOp: make sure the irobot_create node is running.");
  }
  else
  {
